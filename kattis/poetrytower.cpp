		...
		for(int ii=0;ii<s[i].size();ii++) {
			for(int j=0;j<n;j++) {
				for(int jj=0;jj<6;jj++) {
					if(d[j][jj]=='*' || d[j][jj]==s[i][ii]) {
						Flow::add_edge(n*7+id,j*7+jj);
					}
				}
			}
			Flow::add_edge(S,n*7+id);
			id++;
		}
	}
	for(int j=0;j<n;j++) {
		for(int jj=0;jj<6;jj++) {
			Flow::add_edge(j*7+jj,j*7+6);
		}
		Flow::add_edge(j*7+6,T);
	}
	return Flow::flow(S,T) == id;
}
...
int main() {
	...
	for(int bm=1;bm<1<<m;bm++) {
		ans += check(n,m,bm) * fact[__builtin_popcount(bm)];
	}
	cout<<ans<<nl;
