...
namespace Dinic {
	const int N = 1000*2+2;
	const int M = 2*N+5000;
...
int main() {
	...
	Dinic::init();
	for (int i=1; i<=n; i++) {
		Dinic::add_edge(source, 2*i, 1);
		Dinic::add_edge(2*i+1, sink, 1);
	}
	int a, b;
	for (int i=0; i<m; i++) {
		cin >> a >> b;
		Dinic::add_edge(2*a, 2*b+1, 1);
		Dinic::add_edge(2*b, 2*a+1, 1);
	}
	...
	if (flow < n) {
		...
		for (int i=1; i<=n; i++) {
			for (int e=Dinic::first[2*i]; e!=-1; e=Dinic::nxt[e]) {
				if (Dinic::flo[e]>0) {
					cout << Dinic::ep[e^1]/2 << nl;
				}
			}
		}
	}
