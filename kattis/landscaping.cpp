...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            if (grid[i][j] == '.') {
                FLOW::add_edge(source, get(i,j), b);
            } else {
                FLOW::add_edge(get(i,j), sink, b);
            }

            if (i > 0) {
                FLOW::add_edge(get(i,j), get(i-1,j), a);
            }
            if (i < n-1) {
                FLOW::add_edge(get(i,j), get(i+1,j), a);
            }
            if (j > 0) {
                FLOW::add_edge(get(i,j), get(i,j-1), a);
            }
            if (j < m-1) {
                FLOW::add_edge(get(i,j), get(i,j+1), a);
            }
    ...
    while (FLOW::bfs(source, sink)) {
        flow += FLOW::dfs(source, sink, INFLL);
    }
