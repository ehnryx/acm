...
Node merge(const Node& left, const Node& right, int l, int r, bool build) {
  Node res;
  if(!build && (!left.ok || !right.ok)) goto skip_build;
...
void pull(int i) {
  st[i] = merge(st[2*i], st[2*i+1], 2*i, 2*i+1, true);
}
...
Node query(int l, int r, int i=1, int s=0, int e=N-1) {
  ...
  return merge(left, right, 2*i, 2*i+1, false);
}
