			...
			FOR(k,0,pu) {
				For(bm,1<<(2*n)) {
					...
					if (j>0 && abs(dp[i-1][j-1][k][bm]) != INF) {
						if (cur<n) dp[i][j][k][bm] = max(dp[i][j][k][bm], dp[i-1][j-1][k][bm] + standard[j-1]);
						else dp[i][j][k][bm] = min(dp[i][j][k][bm], dp[i-1][j-1][k][bm] - standard[j-1]);
					}
					// take ultimate
					if (k>0 && abs(dp[i-1][j][k-1][bm^1<<cur]) != INF) {
						if (cur<n) dp[i][j][k][bm] = max(dp[i][j][k][bm], dp[i-1][j][k-1][bm^1<<cur] + ultimate[k-1]);
						else dp[i][j][k][bm] = min(dp[i][j][k][bm], dp[i-1][j][k-1][bm^1<<cur] - ultimate[k-1]);
					}
