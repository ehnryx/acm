...
node*  begin(node*c){if(splay(c)!=null)while(fix(c),c->l!=null)c=c->l;return splay(c);}
...
node* nxt(node* x) { if (splay(x)->r == null) return null; x = x->r;
  while (fix(x), x->l != null) x=x->l; return splay(x); }
...
int main() {
	...
	For(i,n) {
		...
		node* cur = new (&data[did++]) node(obj,0);
		enum { last, after, before } insert;
		if (w == null) {
			obj->val = i;
			insert = last;
		} else {
			SplayTree::fixDownTo(w->key);
			if (w->key->val == i-a-1) {
				obj->val = w->key->val+1;
				root[c] = w;
				insert = after;
		...
		ans = SplayTree::insertBefore(ans, SplayTree::nth(ans, obj->val), obj);
		SplayTree::rUpdate(ans, obj, null, 1);
		if (insert == last) {
			root[c] = SplayTree::insertBefore(root[c], null, cur);
		} else if (insert == after) {
			root[c] = SplayTree::insertBefore(root[c], SplayTree::nxt(root[c]), cur);
		} else {
			root[c] = SplayTree::insertBefore(root[c], root[c], cur);
		}
