...
void solve_case() {
  ...
  a.push_back(0);
  vector<int> llen(n+1), rlen(n+1);
  for(int i=1; i<n; i++) {
    llen[i] = 1 + (a[i] == a[i-1] ? llen[i-1] : 0);
  }
  for(int i=n-1; i>0; i--) {
    rlen[i] = 1 + (a[i] == a[i+1] ? rlen[i+1] : 0);
  }
  int ans = max(2 + rlen[2], 2 + llen[n-2]);
  for(int i=0; i<n; i++) {
    if(i > 0) ans = max(ans, 2 + llen[i-1]);
    if(i+1 < n) ans = max(ans, 2 + rlen[i+2]);
    if(0 < i && i+1 < n && (a[i] + a[i+1]) % 2 == 0) {
      int cur = 3;
      if(i-1 > 0 && 2*a[i-1] == a[i] + a[i+1]) cur += llen[i-1];
      if(i+2 < n && 2*a[i+2] == a[i] + a[i+1]) cur += rlen[i+2];
      ans = max(ans, cur);
    }
  }
  cout << ans << nl;
