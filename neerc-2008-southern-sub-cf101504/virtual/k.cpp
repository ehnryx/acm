...
struct Flow {
  ...
  void add_edge(int a, int b, ll c, ll id) {
    nxt[m] = first[ep[m]=a], first[ep[m]] = m, cap[m] = c, eid[m] = id;
    remap[id].push_back(m);
    ++m;
    nxt[m] = first[ep[m]=b], first[ep[m]] = m, cap[m] = c, eid[m] = id;
    remap[id].push_back(m);
    ++m;
  }

  ll mf_update(int s, int t, ll del) {
...
/*
  for (auto it = sat.begin(); it != sat.end(); ) {
    int nonzero = 0, sum = 0;
    for (int j : remap[*it]) {
      nonzero += (flow.flo[j] != 0);
      sum += flow.flo[j];
    }
    if (nonzero > 2) {
      it = sat.erase(it);
