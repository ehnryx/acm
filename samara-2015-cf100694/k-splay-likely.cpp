...
struct splay_tree {
  ...
  node_t* splay_to(node_t* x, node_t* to) {
    if (x == nil) [[unlikely]] return x;
    if (to == nil) [[unlikely]] return splay(x);
    if constexpr (has_push) push_down_to(x);
    _splay_to_rec(x, to);
    pull(x);
    return x;
  ...
  node_t* lower_bound_path(const key_t& key) const {
    if (root == nil) [[unlikely]] return root;
    node_t* u = root;
    while (true) {
      if constexpr (has_push) push(u);
      if (key <= u->key) {
        if (u->left == nil) [[unlikely]] {
          return u;
        }
        u = u->left;
      } else {
        if (u->right == nil) [[unlikely]] {
          return u;
        }
        u = u->right;
      }
  ...
  node_t* next(node_t* u) const {
    if (u == nil) [[unlikely]] throw invalid_argument("next(nil) is not valid");
    if (u->right != nil) {
      if constexpr (has_push) push(u);
  ...
  node_t* prev(node_t* u) const {
    if (u == nil) [[unlikely]] throw invalid_argument("prev(nil) is not valid");
    if (u->left != nil) {
      if constexpr (has_push) push(u);
  ...
  node_t* at(int i) {
    if (i < 0 || i >= size()) [[unlikely]] return nil; // do we want to throw?
    node_t* u = root;
    while (true) {
      if constexpr (has_push) push(u);
  ...
  node_t* push_back(node_t* x) {
    if (root == nil) [[unlikely]] return root = x;
    node_t* at = last();
  ...
  splay_tree split_before(node_t* x) {
    if (splay(x) == nil) [[unlikely]] return dislodge();
  ...
  void append(splay_tree& second) {
    if (second.root == nil) [[unlikely]] return;
    if (root == nil) [[unlikely]] {
      root = second.root;
    } else {
      splay(last());
  ...
  node_t* range(node_t* left, node_t* right) {
    if (left == nil) [[unlikely]] {
      if (right == nil) [[unlikely]] return root;
      else return splay(right)->left;
    } else if (right == nil) [[unlikely]] {
      return splay(left)->right;
    }
    if (left == right) [[unlikely]] return nil;
    splay(left);
...
private:
  ...
  void _splay_rec(node_t* x) {
    if (x->parent == nil) [[unlikely]] return;
  ...
  void _splay_to_rec(node_t* x, node_t* to) {
    if (x->parent == to) [[unlikely]] return;
    if (x->parent->parent == to) [[unlikely]] {
      return _rotate(x);
    }
    _rotate(is_left_child(x) == is_left_child(x->parent) ? x->parent : x);
    _rotate(x);
