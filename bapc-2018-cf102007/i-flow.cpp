...
int main() {
  ...
  For(i,k) {
    ...
    dijkstra(i);
    FOR(j,1,n) {
      vals[i].push_back(pair<ll,int>(dist[i][j],j));
    }
  ...
  function<bool(ll)> valid = [&] (ll t) {
    Dinic::init();
    For(j,k) {
      Dinic::add_edge(S, j+1, cap[j]);
      for (const pair<ll,int>& it : vals[j]) {
        if (it.first > t) break;
        Dinic::add_edge(j+1, k+it.second, INFLL);
      }
    }
    FOR(i,1,n) {
      Dinic::add_edge(k+i, T, p[i]);
    }
