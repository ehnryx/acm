...
pol make_circle(pt c, ld r) {
	vector<ld> ang;
	for (int i=0; i<W; i++) {
		ang.push_back(2*PI*i/W);
	}

	pol out;
	for (int i=2,j=0; j<W; i=j++) {
...
void circle_intersect_out(vector<pol>& nxt, const pol& p, const pt& c, ld r) {
	...
	for (int i=0; i<n; i++) {
		...
		bool ok = false;
		if (it.size() == 1) {
			if (enter(e, inter[0], (inter[0]-c)*pt(0,1))) {
				ok = true;
				first = inter[0];
				start = i+1;
				break;
			...
			if (enter(e, inter[0], (inter[0]-c)*pt(0,1))) {
				assert(!enter(e, inter[1], (inter[1]-c)*pt(0,1)));
				pol add = {
					{ inter[0], inter[1], e.c, 1 },
					{ inter[1], inter[0], c, -1 }
				};
				nxt.push_back(add);
			}
			else if (enter(e, inter[1], (inter[1]-c)*pt(0,1))) {
				ok = true;
				first = inter[1];
				start = i+1;
				break;
	...
	// found first point
	for (int i=0; i<n; i++) {
		const Edge& e = p[(start+i)%n];
		vector<pt> inter = intersect(e, c, r);
		for (const pt& it : inter) {
			if (enter(e, it, (it-c)*pt(0,1))) {
			}
			else {
			}
		}
