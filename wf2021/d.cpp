...
bool seg_x_seg_strict(const pt& a, const pt& b, const pt& c, const pt& d) {
  ld ab = abs(b - a);
  ld cd = abs(c - d);
  int r1 = sgn(ubc::cp(b-a, c-a), ab * EPS);
  int r2 = sgn(ubc::cp(b-a, d-a), ab * EPS);
  int r3 = sgn(ubc::cp(d-c, a-c), cd * EPS);
  int r4 = sgn(ubc::cp(d-c, b-c), cd * EPS);
  return r1 * r2 < 0 and r3 * r4 < 0;
}

bool seg_x_seg_nonstrict(const pt& a, const pt& b, const pt& c, const pt& d) {
  ld ab = abs(b - a);
  ld cd = abs(c - d);
  int r1 = sgn(ubc::cp(b-a, c-a), ab * EPS);
  int r2 = sgn(ubc::cp(b-a, d-a), ab * EPS);
...
void solve_main([[maybe_unused]] int testnum, [[maybe_unused]] auto& cin) {
  ...
  auto is_inside = [&](int i, const pt& to) {
    int a = (i+1 == n ? 0 : i+1);
    int b = (i == 0 ? n-1 : i-1);
    if(ubc::cp(p[a] - p[i], p[b] - p[i]) >= 0) {
      return ubc::cp(p[a] - p[i], to - p[i]) >= -EPS and
        ubc::cp(to - p[i], p[b] - p[i]) >= -EPS;
    } else {
      return not (
          ubc::cp(p[a] - p[i], to - p[i]) < -EPS and
          ubc::cp(to - p[i], p[b] - p[i]) < -EPS);
    }
  };
  ...
  auto is_visible = [&](const pt& a, const pt& b) {
    ...
    for(int i=n-1, j=0; j<n; i=j++) {
      if(seg_x_seg_strict(a, b, p[i], p[j])) {
        return false;
      }
      ld ab = abs(b - a);
      int ci = sgn(ubc::cp(b - a, p[i] - a), ab * EPS);
      int cj = sgn(ubc::cp(b - a, p[j] - a), ab * EPS);
      bool oi = (ubc::dp(b - a, p[i] - a) > ab * EPS and
          ubc::dp(a - b, p[i] - b) > ab * EPS);
      bool oj = (ubc::dp(b - a, p[j] - a) > ab * EPS and
          ubc::dp(a - b, p[j] - b) > ab * EPS);
      if(ci == 0 and cj == 0 and (oi or oj)) {
        if(ubc::dp(p[j] - p[i], b - a) > 0) {
          right = false;
        } else {
          left = false;
        }
  ...
  for(int t=0; t<n; t++) {
    ...
    for(int i=n-1, j=0; j<n; i=j++) {
      ...
      ld ij = abs(p[i] - p[j]);
      if(abs(ubc::cp(statue - p[t], p[j] - p[i])) < ij * EPS) {
        continue;
      }
      if(seg_x_seg_nonstrict(far, p[t], p[i], p[j])) {
        pt it = ubc::line_inter(far, p[t], p[i], p[j]);
    ...
    for(int i=0; i<n; i++) {
      auto c = ubc::lsp_closest(p[t], far, p[i]);
      if(is_inside(i, c) and is_visible(p[i], c)) {
        dist[n][i] = min(dist[n][i], abs(p[i] - c));
      }
    }
    auto c = ubc::lsp_closest(p[t], far, guard);
