...
void solve_case() {
	...
	int n;
  cin >> n;
  auto pc = read_array<pair<int,ll>>(n);
  ll total = accumulate(begin(pc), end(pc), (ll)0,
      [](const auto& x, const auto& y) { return x + y.first * y.second; });

  function<int(ll)> factor = [=](ll v) {
    int sum = 0;
    for(auto [p, c] : pc) {
      while(v % p == 0) {
        v /= p;
