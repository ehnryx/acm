...
ll solve(const vector<vector<Edge>>& adj, const vector<map<int, int>>& edges, int a, int b) {
  if (size(adj[a]) > size(adj[b])) swap(a, b);
  if (auto it = cache.find(pair(a, b)); it != cache.end()) {
    return it->second;
  }
  ll ans = 0;
  for (Edge e : adj[a]) {
    if (e.to == b) {
      ans += 2 * e.cap;
    } else if (auto it = edges[b].find(e.to); it != edges[b].end()) {
      ans += min(e.cap, it->second);
    }
  }
  return cache[pair(a, b)] = ans;
}
