...
void solve_case() {
  ...
  string s;
  cin >> s;
  vector cnt(26, vector<int>(size(s) + 1));
  for(int i=0; i<size(s); i++) {
    cnt[s[i] - 'a'][i + 1] = 1;
  }
  for(int i=0; i<26; i++) {
    partial_sum(begin(cnt[i]), end(cnt[i]), begin(cnt[i]));
  }

  auto count = [&cnt](int i, int l, int r) -> int {
  ...
  while(q--) {
    ...
    for(int split : { m, m + 1 }) {
      int diff = 0;
      for(int i=0; i<26; i++) {
        diff += abs(count(i, l, split-1) - count(i, split, r));
      }
      ok |= (diff == 1);
    }
