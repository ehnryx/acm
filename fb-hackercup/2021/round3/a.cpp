...
void solve_case() {
  ...
  while(m--) {
    ...
    cin >> a >> b;
    if(a > b) swap(a, b);
    auto it = prev(all.upper_bound(pair(a, n + 78)));

    assert(it->first <= a && a <= it->second);
    int cnt = 0;
    int left = it->first;
    while(it->second < b) {
    ...
    int right = it->second;
    assert(it->first <= b && b <= it->second);

    /*
    if (it->first > left && cnt) {
      int len = it->second - it->first + 1;
      have -= (ll)len * (len - 1) / 2;
    }
