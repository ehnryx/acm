...
void solve_case() {
  ...
  function<void(int)> dfs = [&](int u) {
    ...
    dp[u] = max(dp[u], all); // skip current, keep all
    if(big) {
      dp[u] = max(dp[u], one + 1); // get child, keep subtree
    }
    if(size(adj[u]) >= k && 1 + nonzero + size(adj[u]) < sz[u]) {
      dp[u] = max(dp[u], all + 1); // get grandchild, but keep all
    }
    //cerr << u << ": " << dp[u] << nl;
  };
  dfs(1);
