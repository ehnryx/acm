...
using segtree = segment_tree<node, int>;
...
void solve_case() {
  ...
  range_query_tree<segtree, int> st(move(adj), 1);
  ...
  for(int i=1; i<=n; i++) {
    if(size(grp[i]) < 2) continue;
    int lca = grp[i].front();
    for(int j=1; j<size(grp[i]); j++) {
      lca = st.lca(lca, grp[i][j]);
    }
    for(int v : grp[i]) {
      st.update_path(v, lca, false, 1);
    }
  }

  int ans = 0;
  for(int i=2; i<=n; i++) {
    ans += st.query_point(i) == 0;
  }
  cout << ans << nl;
