...
struct node { node *l, *r, *p; ll id, key, val, cnt; S acc; bool rev;
...
node::node(ll i, ll k, ll v) { id=i; key=k; val=v; cnt=1; acc={v,v,v,v}; rev=0;
...
void update(node* x) { if (x != null) { x->cnt = 1 + x->l->cnt + x->r->cnt;
  ...
  x->acc.pref = max(x->l->acc.pref,
                    x->l->acc.sum + x->val,
                    x->l->acc.sum + x->val + x->r->acc.pref);
  x->acc.suff = max(x->r->acc.suff,
                    x->r->acc.sum + x->val,
                    x->r->acc.sum + x->val + x->l->acc.suff);
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (c == 'I') {
            ...
            root = insertBefore(root, nth(root, x-1), new (&data[id++]) node(0, 0, y));

        } else if (c == 'D') {
            int x; scanf("%d %d", &x);
            root = del(nth(root, x-1));

        } else if (c == 'R') {
            int x, y; scanf("%d %d", &x, &y);
            pUpdate(nth(root, x-1), y);

        } else if (c == 'Q') {
            int x, y; scanf("%d %d", &x, &y);
            printf("%d\n", rQuery(root, nth(root, x-2), nth(root, y)).ans);
