...
int main() {
  ...
  for(int i=2; i<=n; i++) {
    int cost;
    cin >> cost;
    Flow::add_edge(i, T, INF, -cost);
  }

  for(int i=0; i<m; i++) {
    int a, b, c, p;
    cin >> a >> b >> c >> p;
    Flow::add_edge(a, b, c, p);
    Flow::add_edge(b, a, c, p);
  }

  Flow::add_edge(S, 1, INF, 0);
  Flow::add_edge(1, T, INF, 0);

  ll ans = 0;
  Flow::min_cost_flow(S, T, ans);
  cout << -ans << nl;
